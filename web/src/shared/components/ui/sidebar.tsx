...
>(
  (
    ...
    const [hoverOverlayOpen, setHoverOverlayOpen] = React.useState(false)
    ...
    // Derive widths directly from React state so the inline style (which has
    // higher specificity than any CSS class) transitions correctly in Tailwind v4.
    const collapsedWidth =
      collapsible === "icon" ? "var(--sidebar-width-icon)" : "0"
    const isHoverOverlay =
      state === "collapsed" &&
      collapsible === "offcanvas" &&
      hoverOverlayOpen
    const desktopStyle = {
      width: state === "collapsed" ? collapsedWidth : "var(--sidebar-width)",
      ...props.style,
    } as React.CSSProperties

    const panelStyle = {
      width: isHoverOverlay
        ? "var(--sidebar-width)"
    ...
    return (
      <>
        ...
        <div
          ...
          style={desktopStyle}
          className={cn(
            "group peer relative hidden h-svh shrink-0 text-sidebar-foreground transition-[width] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] md:block",
            "group-data-[collapsible=offcanvas]:w-0 group-data-[collapsible=icon]:w-[var(--sidebar-width-icon)]",
          ...
          data-side={side}
          onMouseLeave={() => {
            if (isHoverOverlay) setHoverOverlayOpen(false)
          }}
        >
          <div
            data-sidebar="panel"
            ...
            className={cn(
              ...
              isHoverOverlay &&
                "fixed left-0 top-0 z-40 h-svh border-r border-sidebar-border shadow-2xl",
              variant === "floating" || variant === "inset"
            ...
            onMouseEnter={() => {
              if (state === "collapsed" && collapsible === "offcanvas") setHoverOverlayOpen(true)
            }}
            {...props}
          >
            {children}
